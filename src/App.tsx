...
export default function App() {
  ...
  const streakStats = useMemo(() => {
    if (!selectedCat) {
      return computeStreakStats({ recordDates: [], todayYmd: today });
    }
    return computeStreakStats({
      recordDates: history.map((r) => r.date),
      todayYmd: today,
    });
  ...
  const todayStatusDisplay = useMemo(() => {
    ...
    let desc = '';
    let badgeClass = 'bg-orange-50 text-orange-800';
    ...
    if (lapsed) {
      ...
      desc =
        lang === 'zh'
          ? `${petName} 今天照護完成，狀態穩定。`
          : `${petName}'s care is done — looking stable today.`;
    ...
    return { statusLabel, desc, badgeClass };
  }, [
    abnormalNote,
    daily,
    dailyItemsForPet,
  ...
  const quickFillTodayAllNormal = useCallback(() => {
    ...
    setDaily((prev) => {
      ...
      for (const item of dailyItemsForPet) {
        if (next[item.id] !== true) next[item.id] = true;
      }
      if (typeof next.abnormalNote !== 'string' || next.abnormalNote.trim() === '') {
        next.abnormalNote = '';
      }
      // Keep dailyNote as-is; never overwrite user text.
      return next;
    });
    showToast(lang === 'zh' ? '已快速填入今日正常狀態' : 'Filled today as normal', 'success', {
      position: 'top',
      durationMs: 1800,
    });
